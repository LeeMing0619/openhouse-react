...
import {Linking, Dimensions, FlatList, View,TouchableOpacity,PermissionsAndroid,BackHandler,} from 'react-native';
...
class PropertyViewAttendeesScreen extends React.Component {
  ...
  exportCSVFile = () => {
    ...
    this.requestCameraPermission(csvString);
  };
  ...
  async  requestCameraPermission(csvString) {
    try {
      ...
      if (granted === PermissionsAndroid.RESULTS.GRANTED) {
        const pathToWrite = `${RNFetchBlob.fs.dirs.DownloadDir}/property.csv`;
        RNFetchBlob.fs
    ...
    return (
      <View style={{flex: 1}} onLayout={this._onLayout}>
        ...
        <View
          ...
          <Button
            style={[this.Change_BtnStyle(this.state.flag_attend),{borderBottomLeftRadius:6,borderTopLeftRadius:6}]}
          ...
          <Button
            style={[this.Change_BtnStyle(this.state.flag_broker),{borderBottomRightRadius:6,borderTopRightRadius:6}]}
            onPress={this.broker}>
