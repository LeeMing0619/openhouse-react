...
class DashboardService {
  ...
  getMortgage = (accountnum,update) => {
    // alert(accountnum);
    return new Promise((resolve, reject) => {
      axios_instance
        .get(
          `${Constants.BASE_API_URL}/get_mortgage_partners.php/?advertisingid=${accountnum}&update=${update}`,
        )
        .then(res => {
          if (res.status === 200) {
            resolve({data: res.data, IsSuccess: true});
  ...
  UploadProperty = (item,uri) => {
    let bodyFormData = new FormData();
    let filename = item.uniqueid + '.jpg';
    body.append('filetoupload', {
      uri: uri,
      name: filename,
      type: 'image/jpg',
    });
    body.append('objectid', item.uniqueid);
    body.append('phototype', 'p');
    return new Promise((resolve, reject) => {
      axios_instance1
        .post(`${Constants.accounturl}/imageupload.php`, bodyFormData)
        .then(res => {
          if (res.status === 200) {
            resolve({data: res.data, IsSuccess: true});
          } else if (res.status !== 200) {
