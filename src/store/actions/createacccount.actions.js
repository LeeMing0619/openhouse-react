...
export function createnewaccount(firstname,lastname,cellphone,officetelephone,title,email,uniqueid,officename,password,company_logo_url,device,appid) {
    return (dispatch)=>{
        dispatch({
            type:CREATE_NEW_ACCOUNT_START,
        });
        (async() =>{
            let res = await AuthService.authcreateaccount(firstname,lastname,cellphone,officetelephone,title,email,uniqueid,officename,password,company_logo_url,device,appid);
            if(res.IsSuccess){
