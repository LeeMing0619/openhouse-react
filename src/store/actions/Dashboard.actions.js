...
export const POST_PROPERTYPHOTO_START = 'POST_PROPERTYPHOTO_START';
export const POST_PROPERTYPHOTO_SUCCESS = 'POST_PROPERTYPHOTO_SUCCESS';
export const POST_PROPERTYPHOTO_FAILD = 'POST_PROPERTYPHOTO_FAILD';

export const QUESTION_SCREEN = 'POST_PROPERTYPHOTO_FAILD';
...
export function QuestionStatus(item) {
  return (dispatch, getState) => {
    (async () => {
        dispatch({
  ...
  return (dispatch, getState) => {
    ...
    let advertisingid =
      getState().login.account !== null &&
        ...
        : '';
    (async () => {
      let res = await DashboardService.getMortgage(advertisingid, 0);
      if (res.IsSuccess) {
        dispatch({
...
export function UploadProperty(item,uri) {
  return dispatch => {
    // dispatch({
    //   type: POST_PUBLIC_START,
    // });
    ...
    (async () => {
      let res = await DashboardService.UploadProperty(item,uri);
      if (res.IsSuccess) {
        dispatch({
          type: POST_PROPERTYPHOTO_SUCCESS,
          payload: res.data,
        });
      } else {
        dispatch({
